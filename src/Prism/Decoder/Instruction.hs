...
decodeStRI reg func (_, b2, b3, _, _, _) =
    let imm = decodeImm b2 b3 
        in
    func reg imm >> updateIP (1 + immLength imm)
    ...
    case mod of
            ...
            case rm of
                0x06 ->
                    let disp16 = getDisp16 b3 b4
                        ...
                        in
                    fmem mem imm >> updateIP (4 + (immLength imm))
                _ ->
                    let mem = decodeMemSeg rm 0
                        imm = decodeImm b3 b4
                        in
                    fmem mem imm >> updateIP (2 + (immLength imm))
        0x01 -> 
            let disp8 = getDisp8 b3
                mem = decodeMemSeg rm disp8
                imm = decodeImm b4 b5
                in
            fmem mem imm >> updateIP (3 + (immLength imm))
        0x02 ->
            let disp16 = getDisp16 b3 b4 
        ...
        0x03 ->
            let reg = decodeReg rm
                imm = decodeImm b3 b4
                in
            freg reg imm >> updateIP (2 + (immLength imm))
            ...
            case rm of
                0x06 ->
                    let disp16 = getDisp16 b3 b4
                        mem = decodeMemDirect disp16
                        in
                    fmem mem reg >> updateIP 4
                _ ->
                    let mem = decodeMemSeg rm 0
                        in
                    fmem mem reg >> updateIP 2
        0x01 -> 
            let disp8 = getDisp8 b3
                mem = decodeMemSeg rm disp8
                in
            fmem mem reg >> updateIP 3
        0x02 ->
            let disp16 = getDisp16 b3 b4 
                mem = decodeMemSeg rm disp16
                in
